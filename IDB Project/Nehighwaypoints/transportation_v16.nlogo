...
globals[
  ...
  average-speed-t
  acc-rate-taxi
  acc-rate-maxitaxi     ; accident rate for motorcycles
  acc-rate-car     ; accident rate for cars
  acc-rate-pub     ; accident rate for public transit
  acc-taxi-count
  acc-maxitaxi-count    ; count of accidents per decision period for motorcycles
  ...
  inc-taxi-count
  inc-maxitaxi-count    ; count of incidents per decision period for motorcycles
  ...
  default-wait-time-pub;
  default-wait-time-maxitaxi
  default-wait-time-taxi

  p-buy p-ope p-saf p-sec p-com p-tim p-pol p-conv ;Weights for low-income people
  p-buy2 p-ope2 p-saf2 p-sec2 p-com2 p-tim2 p-pol2 p-conv2;Weights for middle-income people
  p-buy3 p-ope3 p-saf3 p-sec3 p-com3 p-tim3 p-pol3 p-conv3;Weights for high-income people
  alpha beta ;Uncertainty Parameters
  commitment
  incoming-commuters

  highway-waypoints ;; NEW
]
...
patches-own[
  ...
  coast-dist;; distance from the sea
]
...
people-own[
  ...
  speed-t
  speed-taxi
  speed-maxitaxi       ; speed affected by congestion for motorcycle
  ...
  safety-taxi
  safety-maxitaxi      ; safety score used for satisfaction function mot
  safety-car      ; safety score used for satisfaction function car
  safety-pub      ; safety score used for satisfaction function
  inc-nw-t-count
  ...
  security-taxi
  security-maxitaxi    ; security score used for satisfaction function mot
  ...
  ;costv-maxitaxi       ; variable cost if the person is using a motorcycle
  costv-car       ; variable cost if the person is using a car
  ;costv-op-maxitaxi    ; score for variable operating cost of motorcycle (variables: gasoline)
  costv-op-car    ; score for variable operating cost of car (variables: gasoline)

  cost-op-taxi
  cost-op-maxitaxi     ; score for total operating cost of mot (the combination of variable and fixed)
  ...
  comfort-taxi
  comfort-maxitaxi     ; comfort score used for satisfaction function mot (percepction towards mot)
  ...
  convinience-taxi
  convinience-maxitaxi ; convinience score used for satisfaction function
  convinience-car ;
  convinience-pub ;
  ...
  time-taxi
  time-maxitaxi        ; time score used for satisfaction function mot
  ...
  satisfaction-maxitaxi; travel satisfaction if the person is using a motorcycle
  satisfaction-taxi
  ...
  w-buy w-ope w-saf w-sec w-com w-tim w-pol w-conv; weights (level of importance) for attributes of transport modes buy= purchase, ope= operation, saf= road safety, sec= personal security, com= comfort, tim= time of travel, pol= pollution, conv= convinience
  ...
  experience-taxi
  experience-maxitaxi  ; # of times that motorcycle has been selected
  ...
  current-target     ;; NEW patch (workplace o waypoint)
  going-highway?     ;; NEW bool
  highway-target     ;; NEW patch del waypoint elegido
  no-sea-counter     ;; NEW
...
to setup
  ...
  set default-wait-time-pub first (item 28 my-csv-dataset) ; defuault waiting time in the public system at normal capacity

  set capacity-maxitaxi   first (item 77 my-csv-dataset) ; number of passengers that the system can move
  set reg-speed-maxitaxi     first (item 78 my-csv-dataset) ; average regular speed of public system with the current capacity
  set default-wait-time-maxitaxi first (item 79 my-csv-dataset) ; defuault waiting time in the public system at normal capacity

  set capacity-taxi   first (item 80 my-csv-dataset) ; number of passengers that the system can move
  set reg-speed-taxi     first (item 81 my-csv-dataset) ; average regular speed of public system with the current capacity
  set default-wait-time-taxi first (item 82 my-csv-dataset) ; defuault waiting time in the public system at normal capacity
...
to setup-highway-waypoints  ;;NEW
  ask waypoints [ die ]

  let coords [[108 143] [108 119] [108 89] [108 70]]
  ...
  set highway-waypoints waypoints
end

to compute-coast-distance
  let sea-patches patches with [ CID = 0 ]

  if not any? sea-patches [
    ask patches [ set coast-dist 999999 ]
    stop
  ]

  ask patches [
    set coast-dist min [ distance myself ] of sea-patches
  ]
end


to-report sees-sea-ahead? ;;NEW
  ;; parches en cono hacia adelante
  let visible-patches patches in-cone vision-dist vision-angle
  report any? visible-patches with [CID = 0]
end

to-report best-highway-waypoint [dest] ;;NEW
  ;; dest es el workplace (patch) del agente
  ;; devuelve un patch (posición del waypoint)
  let me-coast [coast-dist] of patch-here

  let best one-of highway-waypoints
  let best-score 1e18

  ask highway-waypoints [
    let wp patch-here
    let score (w1 * [distance wp] of myself) +
              (w2 * distance dest) +
              (w3 * max list 0 (me-coast - [coast-dist] of wp)) ;; penaliza si el wp NO me aleja del mar
    if score < best-score [
      set best-score score
      set best self
    ]
  ]
  report [patch-here] of best
end
...
; WE NOW USE THE % FROM THE SHP FILE
  ...
  let counter 0
   ...
   [
    ...
    ; In situations when the destination is too small to fit any patches, pick a random patch instead. - Max
    ifelse matching-patches != no-patches
    [
      set workplace one-of matching-patches
    ]
    [
      ; any patch could be one in the middle of the sea?
      ;let land-patches patches with [CID > 0 and CID != target-community]
      ;set workplace one-of land-patches
      set workplace one-of patches
    ]
    ;face workplace

    set dist-home-work (distance workplace)

    set current-target workplace
    set going-highway? false
    set highway-target nobody
    set no-sea-counter 0
...
let prob-m1t first (item 90 my-csv-dataset)  ; male-social-type-1-taxi
...
let prob-m2t first (item 91 my-csv-dataset)  ; male-social-type-2-taxi
...
let prob-m3t first (item 92 my-csv-dataset)  ; male-social-type-3-taxi
...
let prob-f1t first (item 93 my-csv-dataset)  ; female-social-type-1-taxi
...
let prob-f2t first (item 94 my-csv-dataset)  ; female-social-type-2-taxi
...
let prob-f3t first (item 95 my-csv-dataset)  ; female-social-type-3-taxi
...
; Assignation for remanining people with t-type 0
   ...
   let mode-prob random-float 1
    ...
    ifelse-value mode-prob <= 0.4 [1][
      ifelse-value mode-prob <= 0.65 [2][
        ifelse-value mode-prob <= 0.85 [3][4]
    ]
  ]
  ]
...
to set-uncertainty-threshold ; level of uncertainty regarding the transport mode that is accepted by people
 ...
 ask people with [t-type = 2] ; maxitaxis
  [
    let uncertm -1
 ...
 ask people with [t-type = 3] ; pub
  ...
  ask people with [t-type = 4] ; taxis
  [
    let uncertt -1
    while [uncertt < 0 or uncertt > 1]
     [
      set uncertt random-normal Uncert-t (Uncert-t * deviation)
     ]
    ;show uncertc
       set uncertainty-threshold uncertt
   ]
...
to set-satisfaction-threshold ; level of satisfaction that people would like to have with their transport mode ;
  ...
  [
    let satisfactt -1
    while [satisfactt < 0 or satisfactt > 1]
     [
      set satisfactt random-normal Satisf-t (Satisf-t * deviation)
     ]
  ;  show satisfactp
       set satisfaction-threshold satisfactt
  ]
...
to weights-socio-1
  ...
  set p-conv first (item 71 my-csv-dataset)
    ...
    let points-conv -1
    while [points-conv < 0 or points-conv > 100]
     [set points-conv random-normal p-conv (p-conv * 0.1)]

    ; converts points in percentages
    let pointstotal points-buy + points-ope + points-saf + points-sec + points-com + points-tim + points-pol + points-conv
    ...
    set w-conv (points-conv / pointstotal)
...
to weights-socio-2
 ...
 ; Weights for middle-income people
  ...
  set p-conv2 first (item 72 my-csv-dataset)
    ...
    let points-conv2 -1
    while [points-conv2 < 0 or points-conv2 > 100]
     [set points-conv2 random-normal p-conv2 (p-conv2 * 0.1)]

    ; converts points in percentages
    let pointstotal2 points-buy2 + points-ope2 + points-saf2 + points-sec2 + points-com2 + points-tim2 + points-pol2 + points-conv2
    ...
    set w-conv (points-conv2 / pointstotal2)
...
to weights-socio-3
 ...
 ; Weights for high-income people
  ...
  set p-conv3 first (item 72 my-csv-dataset)
    ...
    let points-conv3 -1
    while [points-conv3 < 0 or points-conv3 > 100]
     [set points-conv3 random-normal p-conv3 (p-conv3 * 0.1)]

    ; converts points in percentages
    let pointstotal3 points-buy3 + points-ope3 + points-saf3 + points-sec3 + points-com3 + points-tim3 + points-pol3 + points-conv3
    ...
    set w-conv (points-conv3 / pointstotal3)
...
to set-speed
  ...
  set average-speed-t first (item 83 my-csv-dataset)
  ...
  ask people [
    ...
    set speed-t random-normal average-speed-t (average-speed-t * 0.1)
   ]
...
; Asignation of initial scores (0-1) to the comfort for each transport mode
  ...
  set comfort-t first (item 86 my-csv-dataset)

; Asignation of initial scores (0-1) to the convinience for each transport mode
  set convinience-m first (item 74 my-csv-dataset)
  set convinience-c first (item 75 my-csv-dataset)
  set convinience-p first (item 76 my-csv-dataset)
  set convinience-t first (item 96 my-csv-dataset)

; Gasoline price USD
  set gas-price first (item 12 my-csv-dataset);

; Efficiency of transport mode km/gal
  ;set eff-maxitaxi first (item 13 my-csv-dataset)
  set eff-car first (item 14 my-csv-dataset)

; Emissions
  set emi-maxitaxi first (item 15 my-csv-dataset)
  set emi-car first (item 16 my-csv-dataset)
  set emi-pub first (item 17 my-csv-dataset)
  set emi-taxi first (item 87 my-csv-dataset)

; Accident rate
  set acc-rate-maxitaxi first (item 18 my-csv-dataset)
  set acc-rate-car first (item 19 my-csv-dataset)
  set acc-rate-pub first (item 20 my-csv-dataset)
  set acc-rate-taxi first (item 88 my-csv-dataset)
...
; Insecurity rates of transport modes
  ...
  set insecur-t first (item 89 my-csv-dataset)
...
to commute
  ...
  [
   ...
   ; set density (equiv-cars * scaling-factor / 2) ; using adjusted number
    set density (equiv-cars / ((count people / 10000) * 30) ) ; DENSITY SHOULD BE THE NUMBER OF CARS ON ROADS DIVIDED THE "CAPACITY" OF THE ROAD, I'M CALCULATING IT AS THE NUMBER OF PEOPLE PER PATCH * X TIMES
...
; effect of congestion
       ...
       set speed-maxitaxi ((speed-m * factor ) * accident-congestion )
       set speed-car ((speed-c * factor ) * accident-congestion )
       set speed-pub ((speed-p * factor ) * accident-congestion )
       set speed-taxi ((speed-t * factor) * accident-congestion )


    ;
     if t-type = 1 [set speed speed-car]
     if t-type = 2 [set speed speed-maxitaxi]
     if t-type = 3 [set speed speed-pub]
     if t-type = 4 [set speed speed-taxi]
...
; movement with CID =! 0
    ...
    ;; Si está lo suficientemente cerca del destino final, llega
    ...
    if not arrived? [
      ...
      ;; 1) Detectar mar en visión
      if sees-sea-ahead? [
        set no-sea-counter 0

        ;; activar modo autopista si no está activo
        if not going-highway? [
          set going-highway? true
          set highway-target best-highway-waypoint workplace
          set current-target highway-target
          ;;show "going highway"
          ;;show current-target
        ]
      ]
      if not sees-sea-ahead? [
        set no-sea-counter no-sea-counter + 1
        if going-highway? = false[
          set current-target workplace
        ]
      ]

      ;; 2) Si estoy yendo a waypoint y ya llegué (o estoy muy cerca), cambio a destino final
      if (going-highway? = true) and (distance highway-target <= step-size) [
        move-to highway-target
        set going-highway? false
        set highway-target nobody
        set current-target workplace
        ;;show "in highway"
      ...
      ;; 3) Histeresis: salir del modo autopista si llevo varios ticks sin ver mar
      if (going-highway? = true) and (no-sea-counter >= sea-hysteresis) [
        ;; si ya no estoy en riesgo, vuelvo al destino
        set going-highway? false
        set highway-target nobody
        set current-target workplace
        ;;show "going out highway"
        ;;show current-target
      ]

      ;; 4) Movimiento: orientar hacia current-target, pero evitando el mar
      ;;show "punto 4"
      ;;show current-target
      face current-target

      while [ (patch-ahead step-size = nobody) or ([CID] of patch-ahead step-size = 0) ] [
        rt 45
      ]
      fd step-size
  ]
...
to update-safety
  ...
  if (ticks mod 30) = 1
  [
    set acc-maxitaxi-count 0
    set acc-car-count 0
    set acc-pub-count 0
    set acc-taxi-count 0
    ask people [set safety 0]
  ]
 ...
 ; Probability of having road accidents
  ...
  [
    ...
    [if random-float 1 <= acc-rate-maxitaxi [set safety 1]]

    ; public accidents
    ;Note3 should the acc rate of public transportation be divided by the people in the public vehicle?
    ask people with [t-type = 3  and not arrived?]
    [if random-float 1 <= acc-rate-pub [set safety 1]]

  ;taxi accidents
    ask people with [t-type = 4  and not arrived?]
    [if random-float 1 <= acc-rate-taxi [set safety 1]]

    ; Counts road accidents by mode in the system
    let acc-car count people with [t-type = 1 and safety = 1]
    let acc-maxitaxi count people with [t-type = 2 and safety = 1]
    let acc-pub count people with [t-type = 3 and safety = 1]
    let acc-taxi count people with [t-type = 3 and safety = 1]

    set acc-car-count acc-car-count + acc-car
    set acc-maxitaxi-count acc-maxitaxi-count + acc-maxitaxi
    set acc-pub-count acc-pub-count + acc-pub
    set acc-taxi-count acc-taxi-count + acc-taxi
...
to update-security
 ...
 ; ask people [set security 0] ;Security = 0 means no incidents.
  ...
  [ set inc-maxitaxi-count 0
    set inc-car-count 0
    set inc-pub-count 0
    set inc-taxi-count 0
 ...
 ; Probability of having personal security incidents
  ...
  ask people with [t-type = 4 and not arrived?]
    [if random-float 1 <= insecur-t [set security 1]]
 ...
 ; Checks if people in the network have experienced insecurity incidents
  ask people
   [
      ...
      set inc-nw-t-count count link-neighbors with [t-type = 4 and security = 1]
...
;      set inc-nw-p-count (inc-nw-p-count + inc-nw-p )
  ...
  set inc-maxitaxi-count count people with [security = 1 and t-type = 2]
  set inc-pub-count count people with [security = 1 and t-type = 3]
  set inc-taxi-count count people with [security = 1 and t-type = 4]

;  ;Counts incidents in the system
;  let inc-car (count people with [security = 1 and t-type = 1] )
;  let inc-maxitaxi (count people with [security = 1 and t-type = 2] )
;  let inc-pub (count people with [security = 1 and t-type = 3] )

  ; Accumulates incidents in the system by decision period
;  set inc-maxitaxi-count (inc-maxitaxi-count + inc-maxitaxi)
...
to update-time
  ...
  let maxitaxi-passengers (count people with [t-type = 2])
  let taxi-passengers (count people with [t-type = 4])

  let wait-turns-maxitaxi (maxitaxi-passengers / (capacity-maxitaxi / scale-population))
  let wait-turns-taxi (taxi-passengers / (capacity-taxi / scale-population))
  let wait-turns-public (bus-passengers / (capacity-public / scale-population)) ; indicates the number of turns people need to wait to catch a bus
  ...
  let mean-speed-t mean [speed] of people with [t-type = 4]
  ...
  if mean-speed-t = 0
  [
    set mean-speed-t average-speed-t
  ]
  ...
  [
    ...
    let relative-speed-p (reg-speed-pub / speed-pub ) ; if current speed in the public system is lower than regular speed at the current capacity, people need to wait more
    set wait-time-p ((random-normal  default-wait-time-pub  1) * relative-speed-p  * wait-turns-public)  ; default wait time with the expected congestion at peak hour affected by the difference in speed with the current situation WE SHOULD DIVIDE WAIT TIME BY 30 BECAUSE IT IS PER DECISION PERIOD.
    ...
    let relative-speed-m (reg-speed-maxitaxi / speed-maxitaxi )
    set wait-time-m ((random-normal  default-wait-time-maxitaxi  1) * relative-speed-m  * wait-turns-maxitaxi)

    let relative-speed-t (reg-speed-taxi / speed-taxi )
    set wait-time-t ((random-normal  default-wait-time-taxi  1) * relative-speed-t  * wait-turns-taxi)
    ...
    [
      ...
      [
        ...
        set time-m (time-m + (2 * (mean-speed-c / mean-speed-m)) + wait-time-m / 30) ; dist-home-work / speed-maxitaxi / 100 * 60+ wait-time-m)
        set time-p (time-p + (2 * (mean-speed-c / mean-speed-p)) + wait-time-p / 30) ; dist-home-work / speed-pub / 100 * 60 + wait-time-p)
        set time-t (time-t + (2 * (mean-speed-c / mean-speed-t)) + wait-time-t / 30) ; dist-home-work / speed-taxi / 100 * 60 + wait-time-t)
      ]

      if t-type = 2
      [
        set time-c (time-c + (2 * (mean-speed-p / mean-speed-c)))
        set time-m (time-m + 2 + wait-time-m / 30)
        set time-p (time-p + 2 + wait-time-p / 30)
        set time-t (time-t + 2 + wait-time-t / 30)
      ]
      ...
      [
        ...
        set time-m (time-m + 2 + wait-time-m / 30) ; dist-home-work / speed-maxitaxi / 100 * 60 + wait-time-m)
        set time-p (time-p + 2 + wait-time-p / 30) ; dist-home-work / speed-pub / 100 * 60 + wait-time-p)
        set time-t (time-t + 2 + wait-time-t / 30)
      ]
      if t-type = 4
      [
        set time-c (time-c + (2 * (mean-speed-t / mean-speed-c)))
        set time-m (time-m + (2 * (mean-speed-t / mean-speed-m )) + wait-time-m / 30)
        set time-p (time-p + (2 * (mean-speed-t / mean-speed-p )) + wait-time-p / 30) ; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;[ISA;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;aqui no estoy segura de este bloque
        set time-t (time-t + (2 * (mean-speed-t / mean-speed-c)) + wait-time-t / 30)
      ]
...
to update-scores-tech-attributes
  ...
  [
   ...
   ;set cost-buy-maxitaxi (cost-buy-maxitaxi * (1 - buy-increase-m) )
   ...
   ;let effi-maxitaxi random-normal eff-maxitaxi (eff-maxitaxi * deviation * 2) ; efficiency of motorcycle (kms/galon) normally distributed for each agent
   let effi-car random-normal eff-car (eff-car * deviation * 2) ; efficiency of car (kms/galon) normally distributed for each agent
   ;set costv-maxitaxi (kms / effi-maxitaxi * gas-price)
  ...
  [
   ...
   ;set costv-op-maxitaxi (1 - ((costv-maxitaxi - min-costv) / (max-costv - min-costv))) ; standarization of cost to obtain a score

   set costv-op-car (1 - ((costv-car - min-costv) / (max-costv - min-costv))) ; standarization of cost to obtain a score

   let costv-op-maxitaxi 1; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;ISA[3/4];;;;;;;;;;;;;;;;;;maxitaxi has variable cost?
   let costfix-op-maxitaxi random-normal costf-op-maxitaxi deviation
   let costv-op-taxi 1;
   let costfix-op-taxi random-normal costf-op-taxi deviation
   ...
   let sumcostop (costv-op-maxitaxi + costfix-op-maxitaxi + costv-op-car + costf-op-car + costv-op-pub + costfix-op-pub + costv-op-taxi + costv-op-taxi) ;fixed cost scores come from the initial data

   set cost-op-maxitaxi ((costv-op-maxitaxi + costfix-op-maxitaxi) / sumcostop)
   set cost-op-car ((costv-op-car + costf-op-car) / sumcostop)
   set cost-op-taxi ((costv-op-taxi + costfix-op-taxi) / sumcostop)
 ...
 ; UPDATE SAFETY SCORES according to total number of accidents by mode
   ...
   let sumacc (acc-maxitaxi-count + acc-car-count + acc-pub-count + acc-taxi-count) ; Cumulative sum of accidents during the decision period

    ifelse sumacc > 0
    [

      set safety-maxitaxi (1 - (acc-maxitaxi-count / sumacc))
      set safety-car (1 - (acc-car-count / sumacc))
      set safety-pub (1 - (acc-pub-count / sumacc))
      set safety-taxi  (1 - (acc-taxi-count / sumacc))
    ]
    [set safety-maxitaxi 1 set safety-car 1 set safety-pub 1 set safety-taxi 1] ;if sum-accident = 0, safety score has to be the highest (1)


; UPDATE SECURITY SCORES

; security scores based on incidents in the agent's network
    let sum-incident (inc-nw-m-count + inc-nw-c-count + inc-nw-p-count + inc-nw-t-count)
    ifelse sum-incident > 0
     [ set security-maxitaxi (1 - (inc-nw-m-count / sum-incident))
       set security-car (1 - (inc-nw-c-count / sum-incident))
       set security-pub (1 - (inc-nw-p-count / sum-incident))
       set security-taxi (1 - (inc-nw-p-count / sum-incident))
     ]
     [ set security-maxitaxi 1 set security-car 1 set security-pub 1 set security-taxi 1] ;if sum-incident = 0, security score has to be the highest (1)
 ...
 ; UPDATE POLLUTION SCORES (CO2 emissions)
    ...
    if t-type = 2
      [set pollution (kms * emi-maxitaxi)]
    if t-type = 3
      [set pollution (kms * emi-pub)]
    if t-type = 4
      [set pollution (kms * emi-taxi)]
  ]

    let emissions-car sum [pollution] of people with [t-type = 1]
    let emissions-maxitaxi sum [pollution] of people with [t-type = 2]
    let emissions-pub sum [pollution] of people with [t-type = 3]
    let emissions-taxi sum [pollution] of people with [t-type = 4]
    let sum-emissions (emissions-maxitaxi + emissions-car + emissions-pub + emissions-taxi) ; total emissions of the system

    let max-emi max (list emi-maxitaxi emi-car emi-pub emi-taxi)
  ...
  ask people with [second-half?] [
    ...
    set comfort-maxitaxi (comfort-m * (1 - prob-bad-weather))
    set comfort-taxi (comfort-t * (1 - prob-bad-weather))
    set comfort-car (comfort-c) ; ????????????????
    set comfort-pub (comfort-p * (1 - prob-bad-weather)) ; probability of mechanic-failure - probability of having bad weather
     ...
     if t-type = 2
      [set comfort comfort-maxitaxi]
     if t-type = 3
      [set comfort comfort-pub]
     if t-type = 4
      [set comfort comfort-taxi]
  ]
  ...
  ask people with [second-half?] [
    ...
    ifelse density >= 1                        ; high congestion increases stress, decreasing convinience
     [
        set convinience-maxitaxi (convinience-m * (1 - density * 0.05))
        set convinience-car (convinience-c * (1 - density * 0.02))
        set convinience-pub (convinience-p * (1 - density * 0.04))
        set convinience-taxi (convinience-t * (1 - density * 0.05))
     ]
     [
      set convinience-maxitaxi (convinience-m) ;
      set convinience-car (convinience-c) ;
      set convinience-pub (convinience-p) ;
      set convinience-taxi (convinience-t)
     ]


     if t-type = 1
      [set convinience convinience-car]
     if t-type = 2
      [set convinience convinience-maxitaxi]
     if t-type = 3
      [set convinience convinience-pub]
     if t-type = 4
      [set convinience convinience-taxi]
  ]
  ...
  ask people with [second-half?] [
    ...
    [
      ...
      set time-maxitaxi ( distance home-location / speed-car ) / time-m
      set time-car ( distance home-location / speed-car ) / time-c
      set time-pub ( distance home-location / speed-car ) / time-p
      set time-taxi ( distance home-location / speed-car ) / time-t
    ]
    [
      set time-maxitaxi 1
      set time-car 1
...
to update-satisfaction
  ask people  with [second-half?]
   [
    ...
    set satisfaction-taxi (w-buy * cost-buy-taxi + w-ope * cost-op-taxi + w-saf * safety-taxi + w-sec * security-taxi + w-com * comfort-taxi + w-tim * time-taxi + w-pol * pollution-tot + w-conv * convinience-taxi)  ;;;;;;;;;; corregir
    set satisfaction-car (w-buy * cost-buy-car + w-ope * cost-op-car + w-saf * safety-car + w-sec * security-car + w-com * comfort-car + w-tim * time-car + w-pol * pollution-tot + w-conv * convinience-car)
...
to update-experience
 ask people  with [second-half?]
  [
   ...
   if t-type = 4 [set experience-taxi experience-taxi + 1]
  ]
end
  ...
  [
   ...
   ; calculates the proportion of times that the selected mode has choosen during the simulation time
    ifelse t-type = 1
     ...
     [ifelse t-type = 2
      [set experience experience-maxitaxi]
      [ifelse t-type = 3
        [set experience experience-pub]
        [set experience experience-taxi]]]
    let myexperience (experience / (experience-maxitaxi + experience-car + experience-pub + experience-taxi))
...
to imitation ; transpot-mode chosen according to the most used among links in their net
  ...
  let maxitaxi count link-neighbors with [t-type = 2] / count link-neighbors
  let pub  count link-neighbors with [t-type = 3] / count link-neighbors
  let taxi count link-neighbors with [t-type = 4] / count link-neighbors

  if car = max (list maxitaxi car pub taxi) [set t-type 1 ]
  if maxitaxi  = max (list maxitaxi car pub taxi) [set t-type 2 ]
  if pub  = max (list maxitaxi car pub taxi) [set t-type 3 ]
  if taxi  = max (list maxitaxi car pub taxi) [set t-type 4 ]

  if maxitaxi = car  and maxitaxi = pub and maxitaxi = taxi [set t-type (1 + random 4)]

  if maxitaxi = car  and maxitaxi > pub and maxitaxi > taxi [ifelse random-float 1 < 0.5 [set t-type 1] [set t-type 2]]
  if maxitaxi = pub  and maxitaxi > car and maxitaxi > taxi [ifelse random-float 1 < 0.5 [set t-type 2] [set t-type 3]]
  if maxitaxi = taxi  and maxitaxi > car and maxitaxi > pub [ifelse random-float 1 < 0.5 [set t-type 2] [set t-type 4]]

  if taxi = car  and taxi > pub and taxi > maxitaxi [ifelse random-float 1 < 0.5 [set t-type 4] [set t-type 1]]
  if taxi = pub  and taxi > car and taxi > maxitaxi [ifelse random-float 1 < 0.5 [set t-type 4] [set t-type 3]]
...
to deliberation ;  this is a rational and individual process of selection. The network doesn't intervene. The final selection occurs trhough a logit function.
  ...
  let utility-maxitaxi (beta1 * cost-buy-maxitaxi + beta2 * cost-op-maxitaxi + beta3 * safety-maxitaxi + beta4 * security-maxitaxi + beta5 * comfort-maxitaxi + beta6 * time-maxitaxi + beta7 * pollution-tot + beta8 * convinience-maxitaxi)

  let utility-car (beta1 * cost-buy-car + beta2 * cost-op-car + beta3 * safety-car + beta4 * security-car + beta5 * comfort-car + beta6 * time-car + beta7 * pollution-tot + beta8 * convinience-car)
  ...
  let P2 exp(utility-maxitaxi) / cumulsum
...
to update-ownership
    ...
    ask people [
    if t-type = 1 [set car-owner  1]
    if t-type = 2 [set car-owner  0]
    if t-type = 3 [set car-owner  0]
    if t-type = 4 [set car-owner  0]
;    if t-type = 1 [set maxitaxi-owner 0 set car-owner  1]
;    if t-type = 2 [set car-owner  0 set maxitaxi-owner 1]
;    if t-type = 3 [set car-owner  0 set maxitaxi-owner 0]
  ]
...
"" ""
PENS
"total" 1.0 0 -10899396 true "" "if (ticks mod (30)) = 0 [plot (acc-maxitaxi-count + acc-car-count + acc-pub-count) ]"
...
"" ""
PENS
"maxitaxi" 1.0 0 -16777216 true "" "if ticks > 30 and (ticks mod (30)) = 1 [plot (mean [satisfaction-maxitaxi] of people with [t-type = 1] )]"
